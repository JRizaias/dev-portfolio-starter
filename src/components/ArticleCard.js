...
export function createArticleCard(article, onArticleDetails) {
  ...
  card.addEventListener('click', () => navigateTo('article', article.slug || article.id || article.title));
  card.addEventListener('keypress', (e) => {
    if (e.key === 'Enter' || e.key === ' ') navigateTo('article', article.slug || article.id || article.title);
  });
